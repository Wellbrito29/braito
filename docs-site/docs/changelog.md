...
### Added
- **Transitive staleness via dependency fingerprints** — each note captures its direct deps' hashes at generation time (persisted to `cache/dep-fingerprints.json`); next run treats a file as a cache hit only when the file's own hash AND every dep's hash match; otherwise the note is re-synthesized. Catches stale inferred content like "calls `findBySku`" after the callee is renamed. 12 unit + 3 e2e tests.
- **Quality harness (static mode)** — `tests/quality/qualityHarness.test.ts` runs `generate` against a 6-file curated fixture (`tests/fixtures/quality-harness/`) and asserts per-file regex expectations on `purpose.observed`, `invariants.observed`, and `knownPitfalls.observed`; aggregate floor guards repo-wide averages; runs in <600ms, no LLM or API key required; detects regressions in filename classification, import categorization, cross-signal inferences, AST extractors, and aggregation wiring
