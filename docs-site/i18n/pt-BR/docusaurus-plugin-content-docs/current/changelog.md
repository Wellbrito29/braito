...
- **Síntese `claude-cli` falhava silenciosamente quando o modelo prefixava prosa** — o parser só removia fences markdown, então respostas como `"Looking at this file...\n{...}"` disparavam `JSON Parse error` e caíam silenciosamente no note estático; extraído `parseJsonResponse.ts` que varre cada candidato `{` com contador de chaves ciente de strings e escapes; system prompt reforçado para proibir prefixos em prosa; 10 novos testes em `tests/llm/parseJsonResponse.test.ts`
