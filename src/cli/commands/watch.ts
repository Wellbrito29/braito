...
export async function runWatch(args: { root?: string }): Promise<void> {
  ...
  // Build dependency graph from static parse (no LLM, fast)
  ...
  // Load existing notes from disk (generated by previous `generate` run)
  ...
  for (const file of files) {
    const notePath = path.resolve(notesDir, file.relativePath + '.json')
    try {
      const raw = fs.readFileSync(notePath, 'utf-8')
      noteMap.set(file.path, JSON.parse(raw) as AiFileNote)
    } catch {
      // No existing note — will be generated on first change
    }
  }

  logger.success(`Loaded ${noteMap.size} existing notes. Ready — waiting for changes...`)
